  ...
  mysqlClient.query("select settingValue as min_tag_id from settings where settingKey = 'min_tag_id';",
    function(rows, err) {
      ...
      .then(function() {
        ...
        console.log(getLogDate() + "Closing mysql client err" + err);
        mysqlClient.end();
        return err;
      });
  });
...
function processBags(url) {
  ...
  .then( function(body) {
    ...
    mysqlClient.query("update settings set settingValue = " + body.pagination.min_tag_id + " where settingKey = 'min_tag_id';", function(rows, err) {
      if(err) {
        return d.reject(err);
      }
      ...
      if(bigSqlStr !== "") {
        mysqlClient.query(bigSqlStr, function(rows) {
          if(err) {
            d.reject(err);
          }
