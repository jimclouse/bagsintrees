...
var getSql = function (fileName, renderData) {
    sql = String(fs.readFileSync(path.join(app.get('root'), 'sql', fileName + '.sql')));
    if(mustache) {
        sql = mustache.render(sql, renderData);
    }
    return sql;
  };
...
app.get('/bags/recent', function (req, res) {
    mysqlHandler(getSql('getRecentBags'), res);
});

app.get('/bags/page/:page/:limit', function (req, res) {
    mysqlHandler(getSql('getBags', {limit: req.params.limit}), res);
});

app.get('/bags/map', function (req, res) {
    mysqlHandler(getSql('getBagsForMap'), res);
});
